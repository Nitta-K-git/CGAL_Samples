...
int main()
{
  ...
  Mesh::Property_map<vertex_descriptor,bool> vflag; // プロパティマップ用の変数定義
  boost::tie(vflag, created) = m.add_property_map<vertex_descriptor,bool>("v:flag",false); // 文字列キーと初期値指定
  assert(created);
  // add some names to the vertices
  vflag[v0] = true;
  vflag[v2] = true;
  ...
  for(vertex_descriptor vd : m.vertices()) { 
    std::cout << name[vd] << " @ " << location[vd] << vflag[vd] << std::endl;
  }
